...
func (s *Server) apiEntities(w http.ResponseWriter, r *http.Request) {
	...
	authors, err := s.store.Authors.List(u.ID)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "internal error"})
		return
	}
	collections, err := s.store.Collections.List(u.ID)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "internal error"})
		return
	}
	...
	for _, a := range authors {
		out = append(out, apiEntity{Kind: "author", ID: a.ID, Name: a.Name, URL: "/authors/" + strconv.FormatInt(a.ID, 10)})
	}
	for _, c := range collections {
		out = append(out, apiEntity{Kind: "collection", ID: c.ID, Name: c.Name, URL: "/collections/" + strconv.FormatInt(c.ID, 10)})
	}
	for _, f := range feeds {
		out = append(out, apiEntity{Kind: "feed", ID: f.ID, Name: f.Title, URL: "/feeds/" + strconv.FormatInt(f.ID, 10)})
	}
