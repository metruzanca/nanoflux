...
RETURNING id, user_id, author_id, title, feed_url, home_url, description,
         etag, last_modified, last_polled_at, last_error, next_page_url, kind, scrape_config, poll_interval_sec, enabled, created_at;
...
UPDATE feeds
SET author_id = ?, title = ?, feed_url = ?, home_url = ?, description = ?,
    kind = 'scrape', scrape_config = ?, poll_interval_sec = ?, enabled = ?
WHERE id = ? AND user_id = ?;
